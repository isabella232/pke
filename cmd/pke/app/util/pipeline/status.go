...
func (p *pipelineStatusReporter) ReportStep(phase string, message string) error {
	return p.report(false, false, phase, message)
}

func (p *pipelineStatusReporter) ReportError(phase string, err error) error {
	return p.report(true, false, phase, err.Error())
}

func (p *pipelineStatusReporter) ReportResult(phase string, err error, finished bool) error {
	message := "success"
	if err != nil {
		message = err.Error()
	}
	return p.report(err != nil, finished, phase, message)
}

func (p *pipelineStatusReporter) report(failure bool, finished bool, phase string, message string) error {
	if p == nil || p.disabled {
		return nil
	}
	if p.client == nil {
	...
	now := time.Now()
	req := pipeline.PostPkeNodeStatusRequest{
		Name:      p.nodeName,
	...
	_, err := p.client.ClustersApi.PostPKENodeStatus(context.TODO(), p.orgID, p.clusterID, req)
	return err
}
